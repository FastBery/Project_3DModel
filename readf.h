...
int main()
{
        ...
        float vx, vy, vz;
        std::string mysor;
        std::string kok = "v 58.189599 48.864150 4.766477";
        const char *ckok = kok.c_str();
        sscanf(ckok, "%s%f%f%f", &mysor, &vx, &vy, &vz);
        V.x = vx;
        V.y = vy;
        V.z = vz;
        Vmass.push_back(V);
        std::cout << Vmass[7].x << " " << Vmass[7].y << std::endl;
}
